...
def train_model_old(
    ...
    device,
    checkpoint_path,
    log_path="results/logs/modulo_model_p=73/",
    epochs=20000,
    num_samples=3000,
    lr=0.001,
    hidden_size=64,
    ...
    print("training model with d_model", d_model, "dim_mlp", dim_mlp, "dropout", dropout, "weight_decay", weight_decay, "lr", lr, "alpha", alpha, "reg_pow", reg_pow, "n_token", n_token, "num_samples", num_samples, "scale_weights", scale_weights)
    ...
    # = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay, betas=(0.9, 0.98))  # weight decay needed for grokking acording to Huang et al.
    ...
    # setup tensorboard
    writer = SummaryWriter(log_dir=log_path + f'opt={opt}_h_{d_model}_dd_{dim_mlp}_sw_{scale_weights}_dpt_{dropout}_N_{num_samples}_wd_{weight_decay}_lr_{lr}/')
    ...
    for epoch in range(epochs):
        ...
        # store weights
        #model.to('cpu')
        # checkpoints[f"epoch{epoch}"] = deepcopy(model.state_dict())
        #model.to(device)
    ...
    # put checkpiioints to cpu
    # for k, v in checkpoints.items():
    #     checkpoints[k] = {kk: vv.cpu() for kk, vv in v.items()}

    writer.close()
    # torch.save(checkpoints, checkpoint_path + f'opt={opt}_h={d_model}_dpt={dropout}_N={num_samples}_rp={reg_pow}_wd={weight_decay}_epochs={epoch}_lr={lr}_seed={seed}.pt')
