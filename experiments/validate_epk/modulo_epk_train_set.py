...
# subsample train loader to 2000 samples (from 4000)
rand_ids = torch.randperm(len(train_loader.dataset))[:2000]
train_dataset = torch.utils.data.Subset(train_loader.dataset, rand_ids)

train_loader_sub = torch.utils.data.DataLoader(train_dataset, batch_size=200, shuffle=False)

# store
torch.save(train_loader_sub, experiment_path + "train_loader_sub.pt")

print("Train set size: ", len(train_loader_sub.dataset))

torch.cuda.empty_cache()

preds = []
for i, batch in enumerate(train_loader_sub):
    print(f"Batch {i}")
